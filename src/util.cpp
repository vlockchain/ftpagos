                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No FICTECPagos.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override FICTECPagos.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "FICTECPagos.pid"));
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                if (!fMatch)
                {
                    ...
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("FICTECPagos"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
